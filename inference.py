...
def pred():
    ...
    ohlc_df = ohlc_df.sort_values(by="date", ascending=True, inplace=True)
    ohlc_df = ohlc_df.reset_index(drop=True, inplace=True)
    ohlc_df = ohlc_df.drop(['unix','symbol','Volume BTC', 'Volume USD'], axis=1, inplace=True)
    ohlc_df = ohlc_df.rename(columns = {'date':'Date', 'open':'opening_price','high':'highest_price', 'low':'lowest_price', 'close':'closing_price'}, inplace = True)
